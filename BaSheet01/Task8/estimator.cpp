...
inline double estimate_variance(const double* values, const unsigned int size) {
  double mean_estimate = estimate_mean(values, size);

  double variance_estimate = 0;
  std::for_each(values, values + size,
                [&](double val) {variance_estimate += (val - mean_estimate) * (val - mean_estimate);});
  return variance_estimate / (size - 1);
}

double naive_variance_estimator(const double * values, const unsigned int size) {
  return estimate_variance(values, size);
}
